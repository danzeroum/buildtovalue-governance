#!/usr/bin/env python3
"""
BuildToValue v0.9.5.1 - Enforcement Engine Unit Tests (FINAL CORRECTED)
...
"""
    ...
    def test_yaml_parsing_error_triggers_fallback(self):
        ...
        # ✅ YAML definitivamente inválido (vai causar YAMLError)
        yaml_content = """
metadata:
  version: "test"
...
class TestApplicablePenalties(unittest.TestCase):
    ...
    def test_emotion_recognition_triggers_eu_ai_act_prohibited(self):
        ...
        issues = ["System uses emotion recognition via micro-expressions"]
        classification = self.classifier.classify(issues)
        applicable = self.loader.get_applicable_penalties(classification)

        prohibited = [p for p in applicable if "prohibited" in p["penalty_id"].lower()]
    ...
    def test_proxy_discrimination_triggers_eu_ai_act_high_risk(self):
        ...
        # ✅ FIX: Use keyword EXATA
        issues = ["Proxy discrimination in loan model using zip code"]
        classification = self.classifier.classify(issues)
        applicable = self.loader.get_applicable_penalties(classification)

        high_risk = [p for p in applicable if "high_risk" in p["penalty_id"].lower()]
    ...
    def test_pii_leakage_triggers_gdpr(self):
        ...
        gdpr = [p for p in applicable if "gdpr" in p["penalty_id"].lower()]
    ...
    def test_redlining_triggers_ecoa(self):
        ...
        issues = ["Algorithm engages in redlining by denying loans in certain zip codes"]
        classification = self.classifier.classify(issues)
        applicable = self.loader.get_applicable_penalties(classification)

        ecoa = [p for p in applicable if "ecoa" in p["penalty_id"].lower()]
...
class TestTotalExposureCalculation(unittest.TestCase):
    ...
    def test_eu_penalties_no_stacking(self):
        ...
        penalties = [
            {"jurisdiction": "European Union", "currency": "EUR",
             "min_penalty": 10000000, "max_penalty": 20000000},  # GDPR
            {"jurisdiction": "European Union", "currency": "EUR",
             "min_penalty": 15000000, "max_penalty": 35000000}  # EU AI Act
        ]
    ...
    def test_us_penalties_stacking_allowed(self):
        ...
        penalties = [
            {"jurisdiction": "United States", "currency": "USD",
             "min_penalty": 10000, "max_penalty": 500000},  # ECOA
            {"jurisdiction": "California, United States", "currency": "USD",
             "min_penalty": 2500, "max_penalty": 7500}  # CCPA
        ]
    ...
    def test_mixed_jurisdiction_exposure(self):
        ...
        penalties = [
            {"jurisdiction": "European Union", "currency": "EUR",
             "min_penalty": 15000000, "max_penalty": 35000000},
            {"jurisdiction": "United States", "currency": "USD",
             "min_penalty": 10000, "max_penalty": 500000}
        ]
...
class TestEnforcementDecisions(unittest.TestCase):
    ...
    def test_prohibited_practice_blocks_immediately(self):
        ...
        self.assertIn("CRITICAL", decision.regulatory_impact["executive_summary"])
        self.assertEqual(
            decision.regulatory_impact["total_exposure"]["total_max_eur"],
            35000000
        )
    ...
    def test_medium_risk_conditional_approval(self):
        ...
        decision = self.engine.enforce(
            task, self.system, issues=["Some bias detected in content filtering"]
        )

        self.assertIn(decision.outcome, [Outcome.APPROVED, Outcome.CONDITIONAL])
    ...
    def test_shadow_ai_credential_exposure_blocks(self):
        """Shadow AI credential exposure should block or escalate immediately."""
        task = Task(
            title="Debug API integration",
            description="Unauthorized access with credential leak: sk-proj-abc123xyz456def789ghi012jkl345"
        )
        decision = self.engine.enforce(task, self.system)
...
class TestExecutiveSummaryGeneration(unittest.TestCase):
    ...
    def test_critical_summary_includes_amount(self):
        ...
        summary = self.loader._generate_executive_summary(penalties, total_exposure)
